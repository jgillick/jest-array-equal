...
describe('arrayMatchers', () => {
  ...
  describe('toEqualArrayBy', () => {
    test('array equals array', () => {
      const { pass } = arrayMatchers.toEqualArrayBy([{ id: 1 }, { id: 2 }], 'id', [{ id: 1 }, { id: 2 }]);
      expect(pass).toBe(true);
    });

    test('arrays in different order', () => {
      const { pass } = arrayMatchers.toEqualArrayBy([{ id: 1 }, { id: 2 }], 'id', [{ id: 2 }, { id: 1 }]);
      expect(pass).toBe(true);
    });
    ...
    test('expected has additional item', () => {
      const { pass } = arrayMatchers.toEqualArrayBy([{ id: 1 }, { id: 2 }], 'id', [{ id: 1 }, { id: 2 }, { id: 3 }]);
      expect(pass).toBe(false);
    });

    test('received has additional item', () => {
      const { pass } = arrayMatchers.toEqualArrayBy([{ id: 1 }, { id: 2 }, { id: 3 }], 'id', [{ id: 1 }, { id: 2 }]);
      expect(pass).toBe(false);
    });

    test('entirely different values', () => {
      const { pass } = arrayMatchers.toEqualArrayBy([{ id: 1 }, { id: 2 }], 'id', [{ id: 3 }, { id: 4 }]);
      expect(pass).toBe(false);
    });

    test('received is not an array', () => {
      const { pass } = arrayMatchers.toEqualArrayBy({} as Array<any>, 'id', [{ id: 3 }, { id: 4 }]);
      expect(pass).toBe(false);
    });

    test('expected is not an array', () => {
      const { pass } = arrayMatchers.toEqualArrayBy([{ id: 3 }, { id: 4 }], 'id', {} as Array<any>);
      expect(pass).toBe(false);
    });
  });
});
